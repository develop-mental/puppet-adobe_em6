...
define adobe_em6::instance::replication_queues (
  ...
  exec { "create_${title}_replication_package":
    command => "bash -l -c 'set -e ; ${adobe_em6::params::dir_tools}/aem_bundle_status.rb -a http://localhost:${port}/system/console/bundles.json -u ${aem_bundle_status_user} -p ${aem_bundle_status_passwd}; zip -rq ${package_file_temp} * ; mv -f ${package_file_temp} ${package_install_dest}'",
    provider => "shell",
